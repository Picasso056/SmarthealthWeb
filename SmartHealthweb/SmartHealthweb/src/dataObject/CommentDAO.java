...
/*
 * CLASS CommentDAO
 * Description : class handling database for comment
 */
...
public class CommentDAO {
	...
	public static void addComment(Comment comment)
	{
		...
		try{
			...
			ps.setTimestamp(2, comment.getTimeCreated());
	...
	/*
	 * Function : getCommentWeb
	 * Description : get all comments given details
	 */
	public static ArrayList<Comment> getCommentWeb(int id,String user,Timestamp date)
	{
		DAOUtil data_object = new DAOUtil();
		Connection connection = data_object.getConnection();
		...
		try{
			...
			while(rs.next())
			{
				Comment p = new Comment(rs.getString(1),rs.getTimestamp(2),rs.getString(3),rs.getTimestamp(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8));
				comment.add(p);
