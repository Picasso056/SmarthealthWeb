...
public class EndUserDAO {
	...
	public static void updateEndUser(Person person)
	{
		...
		{
			PreparedStatement ps ;
			ps=connection.prepareStatement("update `smarthealthdb`.`User` set Password=?,Email1=?,Email2=?, FirstName=?, LastName=?, AboutMe=?, PhotoURL1=?, PhotoURL2=?,PhotoURL3=?, StreetNumber=?,StreetName=?, MajorMunicipality=?, GoverningDistrict=?,PostalArea=?,Status=?,UserTypeID=? where Username=?");
		    ...
		    ps.setInt(16, person.getUsertypeid());
		     ps.setString(17,person.getUsername());
		    ...
		    ps=connection.prepareStatement("update `smarthealthdb`.`EndUser` set Karma=? where Username=?");
		    ps.setInt(1,person.getKarma());
